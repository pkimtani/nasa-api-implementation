...
export const getNearEarthObjectsResponseFactory: Factory<NasaApiResponse> =
  Factory.define<NasaApiResponse>(({ transientParams }) => {
    const { date } = transientParams;
    ...
    return {
      ...
      element_count: 1,
      near_earth_objects: nearEarthObjectsFactory.build(
        {},
        { transient: { date } },
      ),
    };
...
const nearEarthObjectsFactory: Factory<NearEarthObjects> =
  Factory.define<NearEarthObjects>(({ transientParams }) => {
    const { date } = transientParams;

    return {
      [date]: nearEarthObjectFactory.buildList(
        faker.datatype.number({ min: 1, max: 10 }),
      ),
    };
  });

const nearEarthObjectFactory: Factory<NearEarthObject> =
